...
CTerminfo::capout_t CTerminfo::Color (EColor fg, EColor bg) const
{
    ...
    if (newAttrs != m_Attrs)
	Attrs (newAttrs); // reuses m_Output
    else
	m_Output.clear();
    if (fg < color_Last)
	RunStringProgram (GetString (ti::set_a_foreground), m_Output, progargs_t(fg));
    if (bg < color_Last)
	RunStringProgram (GetString (ti::set_a_background), m_Output, progargs_t(bg));
    return (m_Output);
}
...
void CTerminfo::_Attrs (uint16_t a) const
{
    const capout_t sgr = GetString (ti::set_attributes);
    if (sgr == string::empty_string) {
	for (uoff_t i = 0; i < attr_Last; ++ i)
	    m_Output += AttrOn (EAttribute (a));
    } else {
	progargs_t pa;
	for (uoff_t i = 0; i < pa.size(); ++ i)
	    pa[i] = (a >> i) & 1;
	RunStringProgram (GetString (ti::set_attributes), m_Output, pa);
	m_Attrs = a;
    }
}

/// Sets all attributes to values in \p a (masked by EAttribute)
CTerminfo::capout_t CTerminfo::Attrs (uint16_t a) const
{
    m_Output.clear();
    _Attrs (a);
    return (m_Output);
...
const CTerminfo::SAcscInfo CTerminfo::c_AcscInfo [acs_Last] = {
    { /* acs_PoundSign */		'}', 'f', 0x00A3 },
    { /* acs_DownArrow */		'.', 'v', 0x2193 },
    { /* acs_LeftArrow */		',', '<', 0x2190 },
    { /* acs_RightArrow */		'+', '>', 0x2192 },
    { /* acs_UpArrow */			'-', '^', 0x2191 },
    { /* acs_Board */			'h', '#', 0x2593 },
    { /* acs_Bullet */			'~', 'o', 0x2022 },
    { /* acs_Checkerboard */		'a', ':', 0x2592 },
    { /* acs_Degree */			'f','\\', 0x00B0 },
    { /* acs_Diamond */			'`', '+', 0x25C6 },
    { /* acs_GreaterEqual */		'z', '>', 0x2265 },
    { /* acs_Pi */			'{', '*', 0x03C0 },
    { /* acs_HLine */			'q', '-', 0x2500 },
    { /* acs_Lantern */			'i', '#', 0x2609 },
    { /* acs_Plus */			'n', '+', 0x253C },
    { /* acs_LessEqual */		'y', '<', 0x2266 },
    { /* acs_LowerLeftCorner */		'm', '+', 0x250C },
    { /* acs_LowerRightCorner */	'j', '+', 0x2510 },
    { /* acs_NotEqual */		'|', '!', 0x2260 },
    { /* acs_PlusMinus */		'g', '#', 0x00B1 },
    { /* acs_Scanline1 */		'o', '~', 0x23BA },
    { /* acs_Scanline3 */		'p', '-', 0x23BB },
    { /* acs_Scanline7 */		'r', '-', 0x23BC },
    { /* acs_Scanline9 */		's', '_', 0x23BD },
    { /* acs_Block */			'0', '#', 0x2588 },
    { /* acs_TopTee */			'w', '+', 0x252C },
    { /* acs_RightTee */		'u', '+', 0x2524 },
    { /* acs_LeftTee */			't', '+', 0x251C },
    { /* acs_BottomTee */		'v', '+', 0x2534 },
    { /* acs_UpperLeftCorner */		'l', '+', 0x2514 },
    { /* acs_UpperRightCorner */	'k', '+', 0x2518 },
    { /* acs_VLine */			'x', '|', 0x2502 }
};
