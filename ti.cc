...
#if BYTE_ORDER == BIG_ENDIAN
    for (auto& i : _numbers) i = le_to_native (i);
    for (auto& i : _stringOffsets) i = le_to_native (i);
#endif
}
...
void CTerminfo::write (ostream& os) const
{
    ...
    for (auto i : _numbers) os << native_to_le (i);
    for (auto i : _stringOffsets) os << native_to_le (i);
...
void CTerminfo::CacheFrequentValues (void)
{
    ...
    if (!acsString.empty()) {
	const auto cFirst = c_AcscInfo, cLast = cFirst + acs_Last;
	for (auto i = acsString.begin(); i < acsString.end(); i += 2)
	    for (auto cFound = cFirst; cFound < cLast; ++cFound)
...
void CTerminfo::LoadKeystrings (keystrings_t& ksv) const
{
	...
	auto ksvp = GetString (ti::EStrings (c_KeyToStringMap [i]));
...
CTerminfo::capout_t CTerminfo::AttrOn (EAttribute a) const
{
	...
	ti::enter_standout_mode,	// a_standout
	ti::enter_underline_mode,	// a_underline
	ti::enter_reverse_mode,		// a_reverse
	ti::enter_blink_mode,		// a_blink
	ti::enter_dim_mode,		// a_halfbright
	ti::enter_bold_mode,		// a_bold
	ti::enter_secure_mode,		// a_invisible
	ti::enter_protected_mode,	// a_protect
	ti::enter_alt_charset_mode,	// a_altcharset
	ti::enter_italics_mode,		// a_italic
	ti::enter_subscript_mode,	// a_subscript
	ti::enter_superscript_mode,	// a_superscript
    };
    _ctx.attrs |= (1 << a);
...
{
    static const ti::EStrings as [attr_Last] = {
	ti::exit_standout_mode,		// a_standout
	ti::exit_underline_mode,	// a_underline
	ti::exit_attribute_mode,	// a_reverse
	ti::exit_attribute_mode,	// a_blink
	ti::exit_attribute_mode,	// a_halfbright
	ti::exit_attribute_mode,	// a_bold
	ti::exit_attribute_mode,	// a_invisible
	ti::exit_attribute_mode,	// a_protect
	ti::exit_alt_charset_mode,	// a_altcharset
	ti::exit_italics_mode,		// a_italic
	ti::exit_subscript_mode,	// a_subscript
	ti::exit_superscript_mode,	// a_superscript
    };
...
void CTerminfo::Attrs (uint16_t a, rstrbuf_t s) const
{
    if (_ctx.attrs == a)
	return;
    const auto sgr = GetString (ti::set_attributes);
    if (sgr == no_value) {
...
CTerminfo::strout_t CTerminfo::Image (coord_t x, coord_t y, dim_t w, dim_t h, const CCharCell* data) const
{
    ...
    const auto oldAttrs (_ctx.attrs);
    const auto oldFg (EColor(_ctx.fg)), oldBg (EColor(_ctx.bg));
	...
	for (coord_t i = x; i < x + w; ++i, ++data) {
		...
		dc = ' ';
	    auto fg (EColor(data->fg)), bg (EColor(data->bg));
...
//}}}-------------------------------------------------------------------
//{{{ c_KeyToStringMap key decoding table

const int16_t CTerminfo::c_KeyToStringMap [kv_nKeys] = {
    ti::key_command,	// kv_Esc
    ti::key_backspace,	// kv_Backspace
    ti::key_btab,	// kv_Backtab
    ti::key_beg,	// kv_Begin
    ti::key_catab,	// kv_CATab
    ti::key_ctab,	// kv_CTab
    ti::key_cancel,	// kv_Cancel
    ti::key_b2,		// kv_Center
    ti::key_clear,	// kv_Clear
    ti::key_eol,	// kv_ClearToEOL
    ti::key_eos,	// kv_ClearToEOS
    ti::key_close,	// kv_Close
    ti::key_command,	// kv_Command
    ti::key_copy,	// kv_Copy
    ti::key_create,	// kv_Create
    ti::key_dc,		// kv_Delete
    ti::key_dl,		// kv_DeleteLine
    ti::key_down,	// kv_Down
    ti::key_c1,		// kv_DownLeft
    ti::key_c3,		// kv_DownRight
    ti::key_end,	// kv_End
    ti::key_exit,	// kv_Exit
    ti::key_f0,		// kv_F0
    ti::key_f1,		// kv_F1
    ...
    ti::key_home,	// kv_Home
    ti::key_ic,		// kv_Insert
    ti::key_il,		// kv_InsertLine
    ti::key_left,	// kv_Left
    ti::key_mark,	// kv_Mark
    ti::key_message,	// kv_Message
    ti::key_mouse,	// kv_Mouse
    ti::key_move,	// kv_Move
    ti::key_next,	// kv_Next
    ti::key_open,	// kv_Open
    ti::key_options,	// kv_Options
    ti::key_npage,	// kv_PageDown
    ti::key_ppage,	// kv_PageUp
    ti::key_previous,	// kv_Previous
    ti::key_print,	// kv_Print
    ti::key_redo,	// kv_Redo
    ti::key_reference,	// kv_Reference
    ti::key_refresh,	// kv_Refresh
    ti::key_replace,	// kv_Replace
    ti::key_restart,	// kv_Restart
    ti::key_resume,	// kv_Resume
    ti::key_right,	// kv_Right
    ti::key_save,	// kv_Save
    ti::key_select,	// kv_Select
    ti::key_sbeg,	// kv_ShiftBegin
    ti::key_scancel,	// kv_ShiftCancel
    ti::key_scommand,	// kv_ShiftCommand
    ti::key_scopy,	// kv_ShiftCopy
    ti::key_screate,	// kv_ShiftCreate
    ti::key_sdc,	// kv_ShiftDelete
    ti::key_sdl,	// kv_ShiftDeleteLine
    ti::key_send,	// kv_ShiftEnd
    ti::key_seol,	// kv_ShiftEndOfLine
    ti::key_sexit,	// kv_ShiftExit
    ti::key_sfind,	// kv_ShiftFind
    ti::key_shelp,	// kv_ShiftHelp
    ti::key_shome,	// kv_ShiftHome
    ti::key_sic,	// kv_ShiftInsert
    ti::key_sleft,	// kv_ShiftLeft
    ti::key_smessage,	// kv_ShiftMessage
    ti::key_smove,	// kv_ShiftMove
    ti::key_snext,	// kv_ShiftNext
    ti::key_soptions,	// kv_ShiftOptions
    ti::key_sprevious,	// kv_ShiftPrevious
    ti::key_sprint,	// kv_ShiftPrint
    ti::key_sredo,	// kv_ShiftRedo
    ti::key_sreplace,	// kv_ShiftReplace
    ti::key_srsume,	// kv_ShiftResume
    ti::key_sright,	// kv_ShiftRight
    ti::key_ssave,	// kv_ShiftSave
    ti::key_ssuspend,	// kv_ShiftSuspend
    ti::key_stab,	// kv_ShiftTab
    ti::key_sundo,	// kv_ShiftUndo
    ti::key_suspend,	// kv_Suspend
    ti::key_undo,	// kv_Undo
    ti::key_up,		// kv_Up
    ti::key_a1,		// kv_UpLeft
